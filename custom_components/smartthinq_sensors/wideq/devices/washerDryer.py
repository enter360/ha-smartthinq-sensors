...
class WMDevice(Device):
    ...
    async def get_energy_last_cycle(self) -> int | None:
        """Get energy per cycle from the most recent day with wash activity.

        Searches the last 7 days for the most recent day that has a non-zero
        cycle count, then returns the average Wh per cycle for that day.
        Returns None if no wash activity is found in the window.
        """
    ...
    async def _get_device_info_v2(self):
        ...
        _LOGGER.debug(
            "Energy poll: today=%s, month=%s, cycle=%s",
            self._energy_today,
...
class WMStatus(DeviceStatus):
    ...
    def energy_last_cycle(self):
        """Return average energy per cycle from the most recent wash day in Wh."""
        return self._energy_feature(
